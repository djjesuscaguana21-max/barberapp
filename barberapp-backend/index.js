...
async function initDB() {
  await pool.query(`CREATE TABLE IF NOT EXISTS servicios (id SERIAL PRIMARY KEY, nombre TEXT, duracion TEXT, precio INTEGER, categoria TEXT, foto TEXT)`)
  await pool.query(`CREATE TABLE IF NOT EXISTS barberos (id SERIAL PRIMARY KEY, nombre TEXT, especialidad TEXT, estrellas REAL, foto TEXT, direccion TEXT, bio TEXT, modo TEXT DEFAULT 'barberia', disponible BOOLEAN DEFAULT false)`)
  await pool.query(`CREATE TABLE IF NOT EXISTS clientes (id SERIAL PRIMARY KEY, nombre TEXT, telefono TEXT, email TEXT, direccion TEXT, foto TEXT)`)
  await pool.query(`CREATE TABLE IF NOT EXISTS citas (id SERIAL PRIMARY KEY, barberia TEXT, servicio TEXT, hora TEXT, fecha TEXT, cliente TEXT, estado TEXT DEFAULT 'confirmada')`)
  await pool.query(`CREATE TABLE IF NOT EXISTS horarios (id SERIAL PRIMARY KEY, barbero_id INTEGER, hora_inicio TEXT, hora_fin TEXT, dias TEXT DEFAULT 'lunes,martes,miercoles,jueves,viernes,sabado')`)
  ...
  if (parseInt(barberosExist.rows[0].count) === 0) {
    await pool.query(`INSERT INTO barberos (nombre, especialidad, estrellas, foto, direccion, bio, modo) VALUES ('Juan Ramirez', 'Especialista en degradê y cortes modernos', 4.9, 'https://images.unsplash.com/photo-1492562080023-ab3db95bfbce?w=300', 'Rua das Flores 123, Diadema SP', 'Barbero profesional con 5 años de experiencia.', 'barberia'), ('Miguel Ferreira', 'Experto en barba y cortes clásicos', 4.6, 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=300', 'Av. Principal 456, Diadema SP', 'Barbero con 3 años de experiencia.', 'domicilio')`)
  }
...
app.put('/servicios/:id', async (req, res) => {
  ...
  const result = await pool.query('UPDATE servicios SET precio = COALESCE($1, precio), nombre = COALESCE($2, nombre), duracion = COALESCE($3, duracion), foto = COALESCE($4, foto) WHERE id = $5 RETURNING *', [precio, nombre, duracion, foto, req.params.id])
  res.json({ mensaje: 'Servicio actualizado', servicio: result.rows[0] })
})

app.post('/servicios', async (req, res) => {
  const { nombre, duracion, precio, categoria, foto } = req.body
  const result = await pool.query('INSERT INTO servicios (nombre, duracion, precio, categoria, foto) VALUES ($1, $2, $3, $4, $5) RETURNING *', [nombre, duracion, precio || 0, categoria, foto || ''])
...
app.put('/barberos/:id', async (req, res) => {
  ...
  const result = await pool.query('UPDATE barberos SET nombre = COALESCE($1, nombre), especialidad = COALESCE($2, especialidad), direccion = COALESCE($3, direccion), bio = COALESCE($4, bio), foto = COALESCE($5, foto), disponible = COALESCE($6, disponible), modo = COALESCE($7, modo) WHERE id = $8 RETURNING *', [nombre, especialidad, direccion, bio, foto, disponible, modo, req.params.id])
...
app.post('/clientes', async (req, res) => {
  ...
  const result = await pool.query('INSERT INTO clientes (nombre, telefono, email, direccion) VALUES ($1, $2, $3, $4) RETURNING *', [nombre, telefono, email, direccion])
  res.json({ mensaje: 'Cliente registrado', cliente: result.rows[0] })
})

app.put('/clientes/:id', async (req, res) => {
  const { nombre, telefono, email, direccion, foto } = req.body
  const result = await pool.query('UPDATE clientes SET nombre = COALESCE($1, nombre), telefono = COALESCE($2, telefono), email = COALESCE($3, email), direccion = COALESCE($4, direccion), foto = COALESCE($5, foto) WHERE id = $6 RETURNING *', [nombre, telefono, email, direccion, foto, req.params.id])
...
app.post('/citas', async (req, res) => {
  ...
  const result = await pool.query('INSERT INTO citas (barberia, servicio, hora, fecha, cliente, estado) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [barberia, servicio, hora, fecha, cliente || 'Cliente', 'confirmada'])
  res.json({ mensaje: 'Cita guardada', cita: result.rows[0] })
})

app.put('/citas/:id', async (req, res) => {
  const { estado } = req.body
  const result = await pool.query('UPDATE citas SET estado = $1 WHERE id = $2 RETURNING *', [estado, req.params.id])
